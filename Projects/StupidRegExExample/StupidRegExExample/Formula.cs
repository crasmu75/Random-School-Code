...
namespace StupidRegExExample
{
	...
	public class Formula
	{
		...
		public IEnumerable<string> tokens; // make private again

		public delegate int Lookup(String expression);
		...
		private object SortToStacks(Func<string, double> lookup)
		{
			...
			{
				Console.WriteLine("going thru tokens");
				// token is variable
				if (varPattern.IsMatch(character))
				{
					Console.WriteLine("matched incorrectly");
				...
				try
				{
					double.Parse(character);
						Console.WriteLine("Matched correctly");
				...
				{
					Console.WriteLine("Uh, didn't work yo");
				}
			...
			Console.WriteLine("done going thru");
			//End of the line
		...
		{
			Console.WriteLine("Is this even being invoked");
			if (type == "intorvar")
			{
				if (operators.Count != 0)
				{
