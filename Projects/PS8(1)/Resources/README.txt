...
11/20 - not counting time (time not spent on this particular assignment)
...
11/25 - about 8 1/2 hours
Mostly everything done, we just need to debug and do testing.
Testing with a GUI client is not working. Switching over to use TelNet. 
Testing on a board we build with the letters AAPOERDSRAVSKEOK so the board can spell
	aardvark for an 11-point word. 
Words we will enter:
- spare +2
- reread +3
- soaker +3
...
- spare +0
- aardvark +0
Finally got all of the functionality working!! Now onto unit testing.
Running out of time. 

Approximately 15 - 16 hours total.
If we had had a bit more time / instruction (we were on the TA queue for quite
a few hours) we could have written more unit testing. We ran into some problems.