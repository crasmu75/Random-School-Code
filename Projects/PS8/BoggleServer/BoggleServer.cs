	...
	class BoggleServer
	{
		...
		public void CommandReceived(String incomingCommand, Exception e, object p)
		{
			if (acceptingCommands)
			{
				...
				if(incomingCommand == null && e == null)
				{
					GameTerminated(ss);
				}
				else
            ...
            lock (ss)
            {
                ...
                else
                {
                    ...
                    if (thirdParamString == null)
                    {
                        ...
                        p1ss.BeginSend("START " + seconds.ToString() + "\n\r" + currentGame.GetPlayerName(currentGame.GetPartner(p1ss)) + "\n\r", Player1ReadyCallback, null);
                        p2ss.BeginSend("START " + seconds.ToString() + "\n\r" + currentGame.GetPlayerName(currentGame.GetPartner(p2ss)) + "\n\r", Player2ReadyCallback, null);
                    }
                    ...
                    else
                    {
						...
						p1ss.BeginSend("START " + thirdParamString + seconds.ToString() + "\n\r" + currentGame.GetPlayerName(currentGame.GetPartner(p1ss)) + "\n\r", Player1ReadyCallback, null);
						p2ss.BeginSend("START " + thirdParamString + seconds.ToString() + "\n\r" + currentGame.GetPlayerName(currentGame.GetPartner(p2ss)) + "\n\r", Player2ReadyCallback, null);
                    }

					// ERASE THISSSSSS
					string letters = currentGame.getBoardLetters();
        ...
        private void ProcessWordCommand(StringSocket ss, string incomingCommand)
        {
            ...
            else
            {
                ...
                string word = incomingCommand.Substring(5).ToUpper().Trim();
        ...
        private void EndGame(Game currentGame, StringSocket player1, StringSocket player2)
        {
            ...
            player1.BeginSend("STOP " + "\n\r" + p1LegalWords.Count.ToString() + " " +
                p1Legal + "\n\r" + p2LegalWords.Count.ToString() + " " + p2Legal + "\n\r" + commonWords.Count.ToString()
                + " " + common + "\n\r" + p1IllegalWords.Count.ToString() + " " + p1Illegal + "\n\r" + p2IllegalWords.Count.ToString()
                + " " + p2Illegal, (ee, pp) => { }, null);

            player2.BeginSend("STOP " + "\n\r" + p2LegalWords.Count.ToString() + " " +
                p2Legal + "\n\r" + p1LegalWords.Count.ToString() + " " + p1Legal + "\n\r" + commonWords.Count.ToString()
                + " " + common + "\n\r" + p2IllegalWords.Count.ToString() + " " + p2Illegal + "\n\r" + p1IllegalWords.Count.ToString()
