            ...
            lock (ss)
            {
                ...
                else
                {
                    ...
                    if (providedBoardLetters == null)
                    {
                        ...
                        p1ss.BeginSend("START " + currentGame.getBoardLetters() + ' ' + seconds.ToString() + ' ' + currentGame.GetPlayerName(currentGame.GetPartner(p1ss)) + '\n', Player1ReadyCallback, null);
                        p2ss.BeginSend("START " + currentGame.getBoardLetters() + ' ' + seconds.ToString() + ' ' + currentGame.GetPlayerName(currentGame.GetPartner(p2ss)) + '\n', Player2ReadyCallback, null);
                    }
                    // Else send the same message and create the same game, but include the 16 letter string specified in
                    //      original args
                    else
                    {
                        currentGame = new Game(p1ss, p2ss, p1name, p2name, seconds, providedBoardLetters);
                        p1ss.BeginSend("START " + providedBoardLetters + " " + seconds.ToString() + ' ' + currentGame.GetPlayerName(currentGame.GetPartner(p1ss)) + '\n', Player1ReadyCallback, null);
                        p2ss.BeginSend("START " + providedBoardLetters + " " + seconds.ToString() + ' ' + currentGame.GetPlayerName(currentGame.GetPartner(p2ss)) + '\n', Player2ReadyCallback, null);
                    }
