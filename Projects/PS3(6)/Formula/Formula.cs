...
namespace SpreadsheetUtilities
{
	...
	public class Formula
	{
		...
		public override bool Equals(object obj)
		{
			...
			if (String.Equals(this.ToString(), obj.ToString()))
				return true;
			else
				return false;
		}
		...
		{
			if (object.Equals(null, f1))
			{
				if (object.Equals(null, f2))
					return true;
				else
					return false;
			}
		...
		{
			int random = 23;
			return random * this.ToString().GetHashCode();
		}
					...
					if (regExOperators.IsMatch(token))
					{
						...
						if (token == "(")
						{
							if (counter == tokens.Count())
								throw new FormulaFormatException("Invalid syntax in expression. A right parenthasis cannot end the expression.");
						...
						{
							if (counter == tokens.Count())
								throw new FormulaFormatException("Invalid syntax in expression. An operator cannot end the expression.");
							if (prevToken == "Operator" && prevToken != "Left Parenthasis")
			...
			foreach (String character in tokens)
			{
				...
				catch (FormatException e)
				{
					if (varPattern.IsMatch(character))
					{
						...
						catch (ArgumentException ex) { return new FormulaError("Variable value not found."); }
		...
		/// This method checks for division by 0 and returns error.
		/// </summary>
